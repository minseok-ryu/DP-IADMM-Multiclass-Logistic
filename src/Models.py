    ...
    for p in range(par.split_number):       
        ...
        for epoch in range(par.num_local_epoch):
            ...
            if par.bar_eps_str != "infty":  
                ...
                sigma = math.sqrt(2*math.log(1.25/1e-6)) * c1 / (par.total_data * float(par.bar_eps_str))
...
####################
def Trial_Second_Block_Problem_ClosedForm(par, x_train_agent, y_train_agent, iteration):  
    ...
    for p in range(par.split_number):       
        ...
        for epoch in range(par.num_local_epoch):
            ...
            par.eta = float(par.a_str) / math.sqrt(iteration+1)  
            Z_Prev = par.Z_val[p]
            Temp = (1.0/(par.rho + (1.0/par.eta)))*(-grad + par.rho*par.W_val + par.Lambdas_val[p] + (1.0/par.eta)*Z_Prev)                    
            Z_Change += np.absolute(Z_Prev - Temp)    
            par.Z_val[p] = bound_constraints(Temp, par.bound) 
